...
import GaleriaD from "./GaleriaD";

export default function SemblanzasD() {
  const [alumnos, setAlumnos] = useState([]);
  const [profes, setProfes] = useState([]);

  useEffect(() => {
    Promise.all([
      fetch("/data/semblanzasD.json").then((response) => response.json()),
      fetch("/data/BaseProfesD.json").then((response) => response.json()),
  ...
  return (
    <div className="container my-12 mx-auto px-4 md:px-12">
      ...
      <img
        ...
        src={`/public/fotos-cursos/portadas/4d.jpg`}
      />
          ...
          <div key={profe.id} className="col-span-2 md:col-span-1">
            ...
            <article className="overflow-hidden rounded-lg shadow-lg bg-white">
              ...
              <footer className="p-2 flex items-center justify-center text-center">
                <Link
                  to={`/semblanzas/4d/profesor/${profe.id}`}
