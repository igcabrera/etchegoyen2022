...
export default function AlumnoDetalleB() {
  const [alumno, setAlumno] = useState({});
  const navigate = useNavigate();
  const { id } = useParams();

  useEffect(() => {
    fetch("/data/semblanzasB.json")
      ...
      <div className="flex justify-center mb-12">
        <div className="w-full max-w-[90%]">
          <div className="bg-white shadow-md rounded-lg px-8 pt-6 pb-8 mb-4">
            ...
            <Link to="/semblanzas/4b" className="mt-6 text-center block py-2 px-4 rounded-lg transition-colors mb-2 bg-secondary hover:bg-primary w-full text-white">Volver</Link>
          </div>
        </div>
      </div>
    </div>
